...
- a provider readback that confirms absence permits retry only when the provider
  also supplies exact idempotency evidence bound to the same operation id and
...
A second test covers the only retryable branch in this model. The provider must
explicitly confirm that no effect exists and bind the same operation id as an
idempotency key to the exact provider/tool/payload fingerprint. Missing or mismatched
idempotency evidence is insufficient.
...
A real integration must define what "authoritative readback" means, how long it
remains valid, the provider's idempotency scope and retention window, and how local
completion is durably reconciled after the remote state is accepted. Until those
properties are known, ambiguity remains a hard stop.
